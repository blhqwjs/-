...
for row in range(2, sheet.max_row + 1):
    name = sheet.cell(row=row, column=1).value  # 假设姓名在第一列
    if name:
        name_length = len(name)
        # 随机选择要替换的字的位置
        num_replacements = random.randint(1, 2)  # 1到2个字进行替换
        indices_to_replace = random.sample(range(name_length), num_replacements)
